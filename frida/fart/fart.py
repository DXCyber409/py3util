...
class CodeItem:
	...
	insarray=""
	idx=0
	def __init__(self,number,methodname, offset, inssize,insarray):
		self.idx=number
...
def parseinsfile():
    ...
    for eachins in insarray:
		...
		ins=eachins[4]
		tempmethod=CodeItem(number,methodname, offset,inssize,ins)
		...
		for i in xrange(0,self.numDirectMethods):	
			...
			if code_off != 0:
				...
				if method != None:
					...
					try:
						print "DirectMethod before repire method+++++++++++++++++++++++++++++++++++\n"
						methodcode = method_code(dex_object, code_off)
						methodcode.printf(dex_object, "\t\t")

						bytearray_str = base64.b64decode(method.insarray)
						print "DirectMethod after repire method++++++++++++++++++++++++++++++++++++\n"
						fixmethodcode = repired_method_code(dex_object, bytearray_str)
						fixmethodcode.printf(dex_object, "\t\t")
					except Exception as e:
						print e

					try:
						# print hex(code_off), binascii.b2a_hex(dex_object.m_content[methodcode.insns:methodcode.insns + methodcode.insns_size * 2])
						print hex(code_off), "\t\t", binascii.b2a_hex(dex_object.m_content[code_off:code_off + method.inssize])
						print hex(method.offset), "\t\t", binascii.b2a_hex(base64.b64decode(method.insarray.encode()))
						# print method.inssize
						bytearray_overrite(code_off, base64.b64decode(method.insarray.encode()))
					except Exception as e:
						print e
		...
		for i in xrange(0,self.numVirtualMethods):			
			...
			if code_off != 0:
				...
				if method != None:
					...
					try:
						print "VirtualMethod before repire method+++++++++++++++++++++++++++++++++++\n"
...
def bytearray_overrite(offset, content):
	global filename_fix_content
	for i in range(0, len(content)):
		filename_fix_content[offset+i] = content[i]
...
class dex_parser:
	...
	def __init__(self,filename):
		...
		for i in xrange(0,self.m_classDefSize):
			str1 = self.getclassname(i)
			dexclass = dex_class(self,i)
			dexclass.printf(self)
			pass
			#self.getclass(i)
...
def main():
	global filename_fix_content
	filename_fix = filename.split('.dex')[0] + '_fix.dex'
	print 'filename_fix:', filename_fix
	shutil.copy(filename, filename_fix)
	filename_fix_f = open(filename_fix, mode='rb+', buffering=4096)
	filename_fix_content = bytearray(filename_fix_f.read())

	dex = dex_parser(filename)

	filename_fix_f.seek(0)
	filename_fix_f.write(filename_fix_content)
	filename_fix_f.flush()
	filename_fix_f.close()
